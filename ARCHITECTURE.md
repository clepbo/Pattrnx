...
| life_area_id | uuid not null → life_areas restrict | |
| activity_type_id | uuid not null → activity_types restrict | the activity created on completion |
...
Constraints: unique `(routine_id, scheduled_date)` where `routine_id is not null`
(idempotent generation, FR-3). Check: `source='routine'` ⇒ `routine_id not null`,
`source='action'` ⇒ `action_id not null`.
...
`goal_id → goals cascade`, `activity_id → activities set null`, `occurred_at`,
`local_date`, `value numeric` (the delta for cumulative goals, the reading for
...
| fingerprint | text not null | hash(detector_key + normalized subject). Unique `(user_id, fingerprint)` |
...
| stale | boolean not null default false | set when source data in window changes (§8.6) |
...
| routines | CRUD, pause/resume, `ensureTasks(user, fromDate, toDate)` (idempotent upsert) |
| tasks | listForDate, complete(variant normal/minimum) via SQL function `complete_task` (task + activity in one transaction, FR-4), skip, reopen |
...
| Routine task materialization (today … today+7) | On Today/goal page load, if the last run was > 1 h ago | `routines.ensureTasks`, an idempotent upsert |
| Pattern detection | Lazy: on Patterns/Today/Review load if `last_detected_at` > 24 h or any pattern is `stale`. Bounded to 90 days | `patterns.detect(user)`, < 2 s budget |
...
Concurrency: detection and generation take a Postgres advisory lock keyed on
`(user_id, job)` via an RPC, so two tabs can't run it twice. Writes are upserts on
unique keys, so a lost race is harmless.
...
≥ 2 milestones and ≥ 10% of the time elapsed. Otherwise Insufficient data.

Output: `{ state, ratio, requiredPace, paceUsed, paceSource: 'historical'|'planned'|null,
gap, adjustments: Adjustment[], explanation: Evidence[] }`. `adjustments`
...
interface Detector {
  ...
  detect(s: UserSnapshot): Candidate[];   // Candidate = subject, observations, effectSize, evidence, consistency
}
```

MVP detectors (the analysis window is 90 days unless noted):

| Key | Kind | What it measures | Min data | Candidate when |
|---|---|---|---|---|
| `frequency.sustainable_rate` | frequency | Actual weekly completions per routine vs planned days/week (Goal Reality Model, blueprint §22) | 4 full weeks of the routine | Median actual/week ≤ 0.7 × planned. Evidence: weeks, planned, done |
| `deviation.overplanning` | deviation | Execution rate on days with > N planned minutes vs ≤ N (N = user's median planned minutes/day) | 20 days with tasks | Rate difference ≥ 20 pp |
| `timing.weekday` | timing | Routine/goal task completion rate by weekday | ≥ 3 observations per weekday involved | Best vs worst weekday differ ≥ 30 pp |
| `timing.hour_band` | timing | For a desired activity type: morning / afternoon / evening share, and completion rate of tasks by `scheduled_time` band | 15 activities | One band holds ≥ 60% of the activity, or completion differs ≥ 25 pp |
| `streak.run_length` | streak | Distribution of consecutive-completion run lengths per routine | ≥ 3 ended runs | Reported as typical run length (median, IQR) |
| `breaking_point.run_end` | breaking_point | Day index where runs end | ≥ 3 ended runs | ≥ 60% of ended runs end within ±1 of the modal index |
| `sequence.checkin_conditioned` | sequence | Next-day completion rate / undesired-activity rate conditioned on a check-in value (workload ≥ 4, sleep < 6 h, stress ≥ 4) | ≥ 6 conditioned days and ≥ 6 unconditioned | Rate difference ≥ 20 pp |
| `sequence.activity_follows` | sequence | P(undesired type B within 24 h after type A) vs base rate of B | A ≥ 6 occurrences | Lift ≥ 1.5 and ≥ 4 co-occurrences |
| `loop.plan_abandon_replan` | loop | Goal/routine created → ≥ 14 days inactive/abandoned → new goal/routine in same life area | 180 days | ≥ 2 complete cycles (BR-3) |

`frequency.sustainable_rate` also produces a concrete plan suggestion: "your
sustainable pace appears closer to 3/week".

### 8.4 Confidence scoring (`engines/patterns/confidence.ts`)

Confidence comes from observation count `n`, `effectSize` (normalized 0–1), and
**split-half consistency**: the detector runs separately on the first and second
half of the window, and the direction must agree.

```text
very_high : n ≥ 30 AND effect ≥ 0.35 AND consistent AND window ≥ 56 days
high      : n ≥ 16 AND effect ≥ 0.25 AND consistent
moderate  : n ≥ 8  AND effect ≥ 0.20
low       : otherwise (stored as candidate, never shown — BR-2)
```

Multiple-comparisons control: at most 3 patterns per review and 1 on Today, ranked
by `confidenceRank × effectSize`, and detectors only test subjects the user
actually planned or marked desired/undesired, which limits the hypothesis space.
All thresholds live in one `thresholds.ts` so the beta can tune them.
...
- Editing or deleting an activity, task or check-in dated inside a visible
  pattern's window sets `stale = true` on that user's patterns overlapping that
  date (a single `update … where window_start <= d and window_end >= d`).
- The next detection run re-evaluates. Candidates that no longer qualify move to
  `resolved`. Feedback and suppression persist by fingerprint.
...
`tests/fixtures/behavior/` holds deterministic generators for synthetic 90-day
users with **planted** patterns: a Wednesday/Thursday drop-off, a Day-4 breaking
point, spending after high-workload days, overplanning, plan-abandon-replan cycles.
There is also a **random-noise** user with no structure. Detector tests assert
that each planted pattern is found at ≥ moderate confidence and that the noise
user produces zero visible patterns. This is how the "no weak correlations as
